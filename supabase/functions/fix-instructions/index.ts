/**
 * fix-instructions — Deno edge function port of /api/fix-instructions.
 *
 * Generates step-by-step turf-management fix instructions for a hole
 * observation, tailored to the course (USDA Zone 5b-6a, bentgrass greens,
 * KBG/PRG fairways).
 *
 * Auth: requires a signed-in user.
 * Secrets needed: ANTHROPIC_API_KEY
 *
 * Deploy:  supabase functions deploy fix-instructions
 */
...
// Inlined from src/lib/hole-constants.ts so this function has no app deps.
...
A ${priority} priority issue has been reported on Hole ${hole_number}:
- Issue: ${title}
- Type: ${issueLabel}
...
Keep it practical and concise — this will be read on a phone screen. Use numbered steps. Don't include a title or header, just the instructions.`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("fix_instructions_api_failure", {
