-- Allow `area` through the pro-shop staff save RPC.
--
-- 20260728120000 added the area discriminator that separates the maintenance
-- crew's schedule from the pro shop's. `save_pro_shop_staff` whitelists its
-- JSONB keys and names columns explicitly in both branches, so all three
-- places need it or a maintenance staff member could not be saved.
--
-- Taken from pg_get_functiondef as it exists in production, with only those
-- additions. COALESCE keeps the pro_shop default when the caller omits area.
...
BEGIN
  ...
  PERFORM public.assert_allowed_jsonb_keys(p_values,ARRAY[
    ...
    'full_name','position','default_group','availability_text','availability','phone','is_active','sort_order','notes','flex','employed_through','area'
  ...
  IF p_staff_id IS NULL THEN
    ...
    INSERT INTO public.pro_shop_staff(id,full_name,position,default_group,availability_text,availability,phone,is_active,sort_order,notes,flex,employed_through,area,created_at,updated_at)
    ...
    VALUES(v_next.id,v_next.full_name,v_next.position,v_next.default_group,v_next.availability_text,v_next.availability,v_next.phone,
      ...
      v_next.is_active,v_next.sort_order,v_next.notes,v_next.flex,v_next.employed_through,COALESCE(v_next.area,'pro_shop'),v_next.created_at,v_next.updated_at) RETURNING * INTO v_next;
  ...
  ELSE
    ...
    UPDATE public.pro_shop_staff SET full_name=v_next.full_name,position=v_next.position,default_group=v_next.default_group,
      ...
      sort_order=v_next.sort_order,notes=v_next.notes,flex=v_next.flex,employed_through=v_next.employed_through,area=COALESCE(v_next.area,'pro_shop') WHERE id=p_staff_id RETURNING * INTO v_next;
