...
- Audit date: 2026-07-15 (America/Chicago)
- Repository baseline: `main` at `d634a60fe7e4c19efb6871277291e364b4fcdd12`
- Live evidence window: 2026-07-15 CDT / 2026-07-16 UTC
...
The operating-system claim fails at the connection layer. Requirements, standards, obligations, My Day checkboxes, schedule-board items, legacy tasks, inspections, meetings, reports, incidents, and corrective actions do not converge on one lifecycle. Today composes several feeds but does not calculate one risk-ranked plan. Standards have a database model and pure proposal rules, but no application/background caller runs those rules. The live database contains 644 generated duty tasks, yet every one is pending and unassigned; there are no task evidence records, verified tasks, temporary coverages, recurrence-history events, or duty audit events. That is a generated backlog, not an operating rhythm.
...
| Security and permissions | 34 | Individual PIN sessions and anonymous lockdown are real improvements; many newer modules retain all-authenticated write policies and sensitive staff/financial domains lack least privilege. |
...
- Inspected 122 application page routes, the 124 pre-existing migrations, 101 pre-existing test files, navigation, hooks, Edge Functions, recurrence engines, storage/security migrations, and recent audit/runbook documents. This session adds one migration and one Vitest file, bringing the reviewed repository to 125 migrations and 102 Vitest files.
...
- Ran `node scripts/security/anon-probe.mjs`: anonymous reads/writes were denied; the authenticated control could read permitted tables.
- Ran `node scripts/audit-data-readiness.mjs`: exact counts and filtered counts only; no row values were printed.
- Five optional filtered counts for evidence/verification requirement-state columns returned non-diagnostic `{ "message": "" }` API errors. They were not used to classify those fields; table-level evidence counts and repository schema were still available.
...
| 644 tasks | Active delegated operating rhythm | All 644 are duty-backed, pending, and unassigned; zero are completed or verified. No evidence or duty audit history exists. |
...
- `20260702_operating_rhythm.sql` made obligations and completions globally writable by any authenticated user. The client directly inserted and deleted completions, allowing actor fields and history to be manipulated unless corrected.
- Sensitive one-on-one notes, engagement profiles, and staff concerns are not manager/private scoped in their defining migrations.
...
- Storage hardening is uneven. `staff-documents` was made private by `20260711_phase0b_b2_staff_documents_private.sql`; the general photos posture and signed-URL/retention model still need bucket-by-bucket verification.
...
- Populated but operationally inactive: 644 duty-backed tasks, all pending and unassigned; 93 evaluations with zero corrective actions; 15 assignments with no executed occurrence.
- Thin: 9 revenue entries, 12 AST inspections, 2 obligation completions, 1 certification, 1 inspection checklist, 1 equipment log, 1 created document, 1 My Day goal/5 steps.
...
- E2E coverage inventories routes rather than exercising the critical operating lifecycles.
...
4. real owner/data cleanup for the current 644 unassigned occurrences and profile departments;
...
## Repository remediation completed with this audit

The implementation slice accompanying this audit adds
`20260716010000_command_center_security.sql`: obligation completions now have
owner/backup/manager authorization, database-derived actors, idempotent command
RPCs, manager-only reasoned correction, preserved baseline snapshots, and an
append-only completion audit. My Day policies become creator-scoped, and the UI
and AI assistant use the same completion command. The migration passed the full
historical local replay and a transactional four-role RLS matrix in a disposable
Supabase database.

This corrects one high-risk repository boundary; it does not raise the production
maturity score because the migration and compatible application code have not
been applied or deployed, and the other Phase 0 security/data blockers remain.
...
| Obligation/My Day repository remediation | `supabase/migrations/20260716010000_command_center_security.sql`; `supabase/tests/command_center_security.sql`; `scripts/test-command-center-security-local.mjs`; `src/__tests__/unit/operations/command-center-security.test.ts` |
| Private-staff RLS gaps | `20260625_calendar.sql`; `20260625_staff_concerns.sql`; `20260714_one_on_one_sessions.sql`; `20260703_certifications.sql` |
