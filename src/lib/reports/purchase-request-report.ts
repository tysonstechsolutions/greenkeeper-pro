...
export async function generatePurchaseRequestReport(
  ...
  try {
    ...
    if (pr.attached_other) {
      // A quote filename has no spaces, so shrinking alone can't save it on a
      // long vendor name — wrap it at the dashes as well.
      setFittedText(form, "Attachment", pr.attached_other, helvetica, written);
    }
