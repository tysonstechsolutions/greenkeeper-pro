/**
 * Order List Report — client-side PDF generator.
 *
 * Ported from /api/reports/order-list-report (Next.js server route) so it
 * runs in the Capacitor build without a Next.js server. jsPDF is already
 * a pure-JS dependency used elsewhere in the app.
 *
 * RLS on `order_items`, `equipment_parts`, and `equipment` already scopes
 * queries to the signed-in user's organization, so moving these reads
 * client-side doesn't widen exposure.
 *
 * Usage:
 *   import { generateOrderListReport } from "@/lib/reports/order-list-report";
 *   const blob = await generateOrderListReport();
 *   // then download via createObjectURL + anchor click
 */
...
export class OrderListReportError extends Error {
  step: string;
  constructor(step: string, message: string) {
    super(message);
    this.step = step;
    this.name = "OrderListReportError";
  }
}

/**
 * Generate the Order List PDF as a Blob.
 *
 * Throws OrderListReportError with a `step` property indicating which
 * stage failed — useful for toast messages and debugging.
 */
export async function generateOrderListReport(): Promise<Blob> {
  let step = "init";
  try {
    ...
    if (orderErr) {
      throw new OrderListReportError(step, orderErr.message);
    }
    ...
    if (partsErr) {
      throw new OrderListReportError(step, partsErr.message);
    }
    ...
    if (items.length === 0 && parts.length === 0) {
      throw new OrderListReportError(
        "no-data",
        "No order items or equipment parts found",
      );
    }
    ...
    const totalCost = items.reduce((sum: number, item: any) => {
      ...
      return sum + (Number(part.estimated_cost) || 0);
    }, 0);
    ...
    for (const category of categories) {
      ...
      doc.text(
        (categoryLabels[category] || category) + " Subtotal: $" + subtotal.toFixed(2),
        pw - m - 80,
        y,
        { align: "right" },
      );
    ...
    if (err instanceof OrderListReportError) throw err;
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`Order list report error at step [${step}]:`, msg, err);
    throw new OrderListReportError(step, msg);
  }
}

/**
 * Convenience wrapper: generate the PDF and trigger a browser download.
 */
export async function downloadOrderListReport(): Promise<void> {
  const blob = await generateOrderListReport();
