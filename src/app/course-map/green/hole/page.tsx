...
function PageContent() {
  ...
  const triggerAutoSave = useCallback((updates: Partial<GreenObservation>) => {
    ...
    <div className="pb-24">
      ...
      <div className="p-4 md:p-6 pb-0">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => router.push(`/course-map/green/measure-at?n=${holeNumber}`)}
        ...
        <div className="flex items-center justify-between mt-2 mb-4">
          {holeNumber > 1 ? (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push(`/course-map/green/hole?n=${holeNumber - 1}`)}
            >
      ...
      {greenObs.length > 0 && (
        <div className="px-4 md:px-6 mb-3 flex gap-2">
          ...
          <Button
            ...
            className="text-xs gap-1.5"
            onClick={() => {
              const subject = encodeURIComponent(`Hole ${holeNumber} Green Observation Report - ${new Date().toLocaleDateString()}`);
              const body = encodeURIComponent(
                `Hi,\n\nPlease find attached the observation report for Hole ${holeNumber} green.\n\nTotal observations: ${greenObs.length}\nOpen issues: ${greenObs.filter(o => o.status !== "resolved").length}\n\nGenerated from VMGC GreenKeeper Pro\n${window.location.origin}/course-map/green/hole?n=${holeNumber}`
              );
