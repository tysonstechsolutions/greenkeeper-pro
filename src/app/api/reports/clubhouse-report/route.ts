...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ══════════════════════════════════════
    // INDIVIDUAL ISSUE PAGES (1 per page)
    // Only for open, in_progress, ordered
    // ══════════════════════════════════════
    step = "pdf-details";
    ...
    for (let idx = 0; idx < detailIssues.length; idx++) {
      ...
      detailRow("Estimated Cost:", issue.estimated_cost != null ? "$" + Number(issue.estimated_cost).toFixed(2) : "—");
    ...
    doc.setFontSize(9);
    doc.setTextColor(...GRAY_600);
    doc.text(`Total Issues: ${items.length}  |  Open: ${open}  |  In Progress: ${inProgress}  |  Ordered: ${ordered}  |  Completed: ${completed}  |  Urgent: ${urgent}`, m + 4, y);
    ...
    const fname = "vmgc-clubhouse-report-" + new Date().toISOString().slice(0, 10) + ".pdf";
