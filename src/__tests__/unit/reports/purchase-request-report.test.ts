...
/**
 * The drawn lines put back together. A line the field wrapped after a dash
 * (a quote filename has no spaces to wrap at) continues without a space.
 */
function joinDrawn(lines: string[]): string {
  return lines.reduce((out, line, i) => (i === 0 ? line : out.endsWith("-") ? out + line : `${out} ${line}`), "");
}
...
describe("quote filename in IGE Based On and the Other box", () => {
  ...
  it("prints the whole label in both fields", async () => {
    ...
    expect(joinDrawn(drawnText(doc, "IGE").lines)).toBe(label);
    expect(joinDrawn(drawnText(doc, "Attachment").lines)).toBe(
      `${label} and SOW`,
    );
  });
