...
/**
 * Execute the private-staff RLS matrix only against the disposable local
...
const sql = readFileSync(
  join(root, "supabase", "tests", "staff_privacy_security.sql"),
  "utf8",
);
const container = process.env.STAFF_PRIVACY_DB_CONTAINER
...
if (!/^supabase_db_[a-zA-Z0-9_.-]+$/.test(container) || container.includes(productionProjectRef)) {
  throw new Error(`Refusing staff-privacy integration test against non-local container: ${container}`);
}
for (const [name, value] of Object.entries(process.env)) {
  if (value?.includes(productionProjectRef) && /(SUPABASE|PROJECT_REF|DATABASE_URL)/i.test(name)) {
    throw new Error(`Refusing staff-privacy integration test: ${name} references production.`);
  }
}
...
try {
  ...
  throw new Error(`Disposable staff-privacy integration test failed\n${stdout}\n${stderr}`);
}
